...
public class RepeaterItem extends TileItem {
	public RepeaterItem(String name, Sprite sprite, String model, String... validTiles) {
	...
	public boolean interactOn(Tile tile, Level level, int xt, int yt, Player player, Direction attackDir) {
		for (String tilename : validTiles) {
			if (tile.matches(level.getData(xt, yt), tilename)) {
				...
				level.setData(xt, yt, player.dir.getDir() << 4); // TODO Applying this when updating the codes.
	...
	public RepeaterItem clone() {
		return new RepeaterItem(getName(), sprite, model, validTiles.toArray(new String[0]));
	}
}
